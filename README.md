  ...
  {family}/            # e.g. Claude, GPT, GLM, DeepSeek…
    {model}/
      {reasoning}/     # off | low | medium | high
        ...
        statistics.json  # always written — even on error/timeout/cancel
...
- child processes get an empty environment (`PATH`/`HOME`/`TMPDIR` inside the
  workspace only), a per-command timeout, and the whole process group is
  killed on expiry;
...
`~/.config/lmhub/config.toml` (all optional):
